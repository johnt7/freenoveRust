...
use esp_wifi_sys_esp32s3::include::{
    esp_err_t, esp_wifi_get_mode, wifi_mode_t, wifi_mode_t_WIFI_MODE_NULL,
};
...
fn wifi_sys_probe_example() {
    // This probes the current Wi-Fi mode through raw ESP-IDF bindings.
    let mut mode: wifi_mode_t = wifi_mode_t_WIFI_MODE_NULL;
    let err: esp_err_t = unsafe { esp_wifi_get_mode(&mut mode as *mut wifi_mode_t) };

    println!(
        "esp-wifi-sys example: esp_wifi_get_mode -> err={} ; mode={}",
        err, mode
    );
}
